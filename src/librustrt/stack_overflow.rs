...
pub fn report(task_name: Option<&str>) {
    ...
    rterrln!("\ntask '{}' has overflowed its stack", task_name.unwrap_or("<unknown>"));
}

// get_task_info is called from an exception / signal handler.
// It borrows the current task from local_ptr and the returned
// 'static str lives as long as the current task. We'll exit the
//  process without destroying tasks soon after this is called.
#[cfg(any(windows, target_os = "linux", target_os = "macos"))]
unsafe fn get_task_info() -> Option<(uint, Option<&'static str>)> {
    use core::prelude::*;
    use local::Local;
    use task::Task;
...
mod imp {
    ...
    use stack;
    use super::{Handler, get_task_info, report};
...
mod imp {
    ...
    unsafe extern fn signal_handler(signum: libc::c_int,
        ...
        // See: https://www.gnu.org/software/libc/manual/html_node/Handler-Returns.html#Handler-Returns
